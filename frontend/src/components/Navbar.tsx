...
export const Navbar: React.FC<NavbarProps> = ({ onMenuToggle, isSidebarOpen }) => {
  ...
  useEffect(() => {
    const syncUser = () => setCurrentUser(session.getUser());
    syncUser();
    window.addEventListener("agritech-auth-change", syncUser);
    return () => window.removeEventListener("agritech-auth-change", syncUser);
  }, []);
            ...
            {showProfile && (
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <Button variant="outline" size="sm" className="w-full justify-start text-destructive gap-2" onClick={handleSignOut}>
                    <User className="h-4 w-4" />
                    {t('Sign Out')}
                  </Button>
                </div>
