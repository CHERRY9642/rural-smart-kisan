...
const Signup = () => {
  ...
  const onSubmit = async (data: SignupForm) => {
    try {
      ...
      navigate(data.plan === 'free' ? '/dashboard' : '/profile');
    } catch (error) {
      toast({
        title: translateSync("Signup Failed"),
        description: translateSync(error instanceof Error ? error.message : "Please try again later."),
        variant: "destructive",
      });
    }
  };
